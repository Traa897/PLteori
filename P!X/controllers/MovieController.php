...
class MovieController {
    ...
    public function index() {
        ...
        require_once __DIR__ . '/../views/movie/index.php';
    }

    // Tampilkan form tambah film
    public function create() {
        $genres = $this->genre->readAll()->fetchAll(PDO::FETCH_ASSOC);
        require_once __DIR__ . '/../views/movie/create.php';
    }
    ...
    public function edit() {
        if(isset($_GET['id'])) {
            ...
            if($this->movie->readOne()) {
                $genres = $this->genre->readAll()->fetchAll(PDO::FETCH_ASSOC);
                require_once __DIR__ . '/../views/movie/edit.php';
            } else {
    ...
    public function show() {
        if(isset($_GET['id'])) {
            $this->movie->id = $_GET['id'];
            if($this->movie->readOne()) {
                require_once __DIR__ . '/../views/movie/show.php';
            } else {
                header("Location: index.php?error=Film tidak ditemukan");
                exit();
            }
